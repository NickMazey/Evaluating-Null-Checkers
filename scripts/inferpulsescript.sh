...
infer run --quiet --debug --no-default-checkers --pulse -- mvn compile -Dbenchmark=$1 -Djavaver=11 -Dinferlocation=$infer_location -Dcheckerframework=false --log-file $log_location/$script_name/$bench_name.log
end_seconds=$( date '+%s' )
end_nanos=$( date '+%N' )
