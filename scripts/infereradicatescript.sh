...
infer run --quiet --debug --no-default-checkers --eradicate -- mvn compile -Dbenchmark=$1 -Djavaver=11 -Dinferlocation=$infer_location -Dcheckerframework=false --log-file $log_location/$script_name/$bench_name.log
end_seconds=$( date '+%s' )
end_nanos=$( date '+%N' )
./scripts/calctime.sh $start_seconds $start_nanos $end_seconds $end_nanos > $log_location/$script_name/$bench_name.time
#Suppresses output
